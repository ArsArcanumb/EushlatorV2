...
def extract_S5IN(fd, out_folder: Path):
    pos = 0x200
    ...
    for i in range(arc_count):
        ...
        for x in range(arc_entry_count):
            ...
            out_path = out_folder / arc_name / file_name
...
def extract_S5IC(fd, is_append, file_filter="", in_folder: Path = None, out_folder: Path = None):
    def extract_file(queue_index):
        ...
        out_arc_path = Path(out_folder) / arc_path.stem
    ...
    for i in range(arc_count):
        ...
        open_archives[i] = {"path": in_folder / name, "file": FileWrapper(in_folder / name)}
...
def extract_archive(archive_path: Path, out_folder, filter):
    archive_folder = archive_path.parent.resolve()
    ...
    if is_compressed:
        extract_S5IC(fd, magic[2] == "A", filter, archive_folder, out_folder)
    else:
        extract_S5IN(fd, out_folder)


###########################################################################


def main(archive_folder, output_folder, filter=""):
    file_folder = Path(archive_folder)
    for file_name in os.listdir(archive_folder):
        file_path = file_folder / file_name
        if file_name == "SYS5INI.BIN" or (file_name.startswith("APPEND") and file_name.endswith(".AAI")):
            extract_archive(file_path, Path(output_folder), filter)
